...
function pegarTextoColor(miTXT) {  
    ...
    var pos = -1;
    for (var i = 0; i < arrayNombresColores.length; i++) {
        if (miTXT == arrayNombresColores[i].toLowerCase()) {
            pos = i;
        }
    }
    // color RGB
    ...
    // color HSL
        ...
        miHex = convertirRGBaHexadecimal(miRGB);
    // color por nombre estándar en inglés
    } else if ( pos != -1 ) {
        miHex = "#" + arrayHexColores[pos];
    // color Hexadecimal
